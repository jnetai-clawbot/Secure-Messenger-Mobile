    ...
    fun sendMessage(contactId: String, text: String) {
        viewModelScope.launch {
            try {
                ...
                app.p2pManager?.sendMessage(contactId, content.toByteArray(Charsets.UTF_8))
    ...
    fun sendFile(contactId: String, file: File) {
        viewModelScope.launch {
            try {
                ...
                com.jnetaol.securemessenger.data.FileStorageManager.saveFile(getApplication(), file.name, fileBytes)
